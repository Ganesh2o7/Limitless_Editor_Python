...
class Editor(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.control = stc.StyledTextCtrl(panel1, style=wx.TE_MULTILINE | wx.TE_WORDWRAP)
        self.control1 = stc.StyledTextCtrl(panel1, style=wx.TE_MULTILINE | wx.TE_WORDWRAP)
        ...
        filemenu = wx.Menu()
        menuNew=wx.MenuItem(filemenu,wx.ID_NEW,'&New\tCtrl+N',kind=wx.ITEM_NORMAL)
        menuNew.SetBitmap(wx.Bitmap('Resources/newfile.png'))
        filemenu.Append(menuNew)

        menuOpen =wx.MenuItem(filemenu,wx.ID_OPEN, '&Open\tCtrl+O', "Open an Existing Document")
        menuOpen.SetBitmap(wx.Bitmap('Resources/openfile.png'))
        filemenu.Append(menuOpen)

        menuSave = wx.MenuItem(filemenu,wx.ID_SAVE, '&Save\tCtrl+S', "Save the Current Document")
        menuSave.SetBitmap(wx.Bitmap('Resources/save.png'))
        filemenu.Append(menuSave)

        menuSaveAs = wx.MenuItem(filemenu,wx.ID_SAVEAS,'&Save As\tCtrl+Shift+S', "Save the New Document")
        menuSaveAs.SetBitmap(wx.Bitmap('Resources/saveas.png'))
        filemenu.Append(menuSaveAs)

        filemenu.AppendSeparator()
        menuClose = wx.MenuItem(filemenu,wx.ID_EXIT, '&Exit\tCtrl+Alt+X', "Close the Application")
        menuClose.SetBitmap(wx.Bitmap('Resources/exit.png'))
        filemenu.Append(menuClose)

        editmenu = wx.Menu()
        menuUndo = wx.MenuItem(editmenu,wx.ID_UNDO, "&Undo\tCtrl+Z", "Undo Last Action")
        menuUndo.SetBitmap(wx.Bitmap('Resources/undo.png'))
        editmenu.Append(menuUndo)
        menuRedo = wx.MenuItem(editmenu,wx.ID_REDO, "&Redo\tCtrl+R", "Redo Last Action")
        menuRedo.SetBitmap(wx.Bitmap('Resources/redo.png'))
        editmenu.Append(menuRedo)
        editmenu.AppendSeparator()
        menuCut = wx.MenuItem(editmenu,wx.ID_CUT, '&Cut\tCtrl+X', "Cut Selected Text")
        menuCut.SetBitmap(wx.Bitmap('Resources/cut.png'))
        editmenu.Append(menuCut)
        menuCopy = wx.MenuItem(editmenu,wx.ID_COPY, "&Copy\tCtrl+C", "Copy Selected Text")
        menuCopy.SetBitmap(wx.Bitmap('Resources/copy.png'))
        editmenu.Append(menuCopy)
        menuPaste = wx.MenuItem(editmenu,wx.ID_PASTE, "&Paste\tCtrl+V", "Paste the Text from Clipboard")
        menuPaste.SetBitmap(wx.Bitmap('Resources/paste.png'))
        editmenu.Append(menuPaste)
        formatmenu = wx.Menu()
        menuFont = wx.MenuItem(formatmenu,wx.ID_ANY, "&Font\tCtrl+F", "Change the Font Settings")
        menuFont.SetBitmap(wx.Bitmap('Resources/font.png'))
        formatmenu.Append(menuFont)

        runmenu = wx.Menu()
        menuRunJava = wx.MenuItem(runmenu,wx.ID_ANY, "&Run Java Program\tShift+J", "Run Java Program")
        menuRunJava.SetBitmap(wx.Bitmap('Resources/jar.png'))
        runmenu.Append(menuRunJava)
        menuRunC = wx.MenuItem(runmenu,wx.ID_ANY, "&Run C Program\tShift+C", "Run C Program")
        menuRunC.SetBitmap(wx.Bitmap('Resources/class.png'))
        runmenu.Append(menuRunC)

        prefmenu = wx.Menu()
        menulinenumbers = wx.MenuItem(prefmenu,wx.ID_ANY, "&Toggle Line Numbers", "Show/Hide Line Numbers Column")
        menulinenumbers.SetBitmap(wx.Bitmap('Resources/options.png'))
        prefmenu.Append(menulinenumbers)

        helpmenu = wx.Menu()
        menuhowto = wx.MenuItem(helpmenu,wx.ID_ANY, "&How To..\tCtrl+H", "Get Help Using the Editor")
        menuhowto.SetBitmap(wx.Bitmap('Resources/how.png'))
        helpmenu.Append(menuhowto)
        menuabout = wx.MenuItem(helpmenu,wx.ID_ABOUT, "&About\tShift+A", "Read About Editor and its Making")
        menuabout.SetBitmap(wx.Bitmap('Resources/about.png'))
        helpmenu.Append(menuabout)

        voicemenu = wx.Menu()
        menuvaish = wx.MenuItem(voicemenu,wx.ID_ANY, "&Female\tCtrl+Shift+F", "Voice")
        menuvaish.SetBitmap(wx.Bitmap('Resources/female.png'))
        voicemenu.Append(menuvaish)
        menuvaish2 = wx.MenuItem(voicemenu,wx.ID_ANY, "&Male\tCtrl+Shift+M", "voice")
    ...
    def sug(event,e):
        ...
        if aa>0.0:
            a = str(event.control.GetRange(float(int(aa - 1.0)),float(event.control.LastPosition)))

        else:
            a = str(event.control.GetRange(float(int(aa - 0.1)), float(event.control.LastPosition)))
    ...
    def copy(event,e):
        ...
        a = event.control1.GetRange(float(event.control1.GetCurrentPos()), float(event.control1.LastPosition))
        ls = a.split("\n")
    ...
    def save_as(self, e):
        ...
        mask = [("Text and Python files", "*.txt *.py "),
                ...
                ("HTML files", "*.htm"),
                ("All files", "*.*")]
        dig = wx.FileDialog(self, "Save File As", initial_dir, "Untitled", "*.*", wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
        if (dig.ShowModal() == wx.ID_OK):
            name = dig.GetFilename()
            dirname = dig.GetDirectory()
            text = str(self.control.GetValue())
    ...
    def exitmethod(self, e):
        m = wx.MessageDialog(self, "Confirm Exit", "Are you sure you want to exit?", wx.YES_NO | wx.ICON_QUESTION)
    ...
    def newproj(self, e):
        m = wx.MessageDialog(self, "Confirm Exit", "do you want save the existing file?", wx.YES_NO | wx.ICON_QUESTION)
